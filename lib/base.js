...
Base.extend = function extend(destination, source){
	...
	switch (ActiveRecord.primary_key_prefix_type){
		case 'table_name': 
			destination.table_name = pluralize(destination.name.toLowerCase()) + 'id'; 
			break;
		case 'table_name_with_underscore': 
			destination.table_name = foreign_key(underscore(destination.name.toLowerCase())); 
			break;
		default: 
			destination.table_name = 'id';
			break;
	}

	if (ActiveRecord.pluralize_table_names) 
		destination.table_name = pluralize(underscore(destination.name.toLowerCase()));
