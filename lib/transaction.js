	...
	this.exec = function exec(sql, params, callback){
		...
		return conn.begin(function _begin(err, tx){
			...
			if (self.connection.get('driver') == 'postgres'){
				var parameters = ActiveSupport.clone(params, []);
				var extraParam;

				if (sql.match(/UPDATE/)){
					parameters = ActiveSupport.clone(params[0], {});
					extraParam = ActiveSupport.clone(params[1], {});
					params = params[0];
				}
				...
				if (extraParam) params = [].concat(params, extraParam);

				sql = Query.queryFormat(sql, params);
				sql = sql.replace(/`/g, '')
				if (!sql.match(/SELECT/)) sql = sql.replace(/'/g, '');

				sql = pgFormat.apply(pgFormat, ActiveSupport.flatten([sql, ActiveSupport.values(parameters)]));
				params = [];
			}
