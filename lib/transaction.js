...
function Transaction (){
	...
	*/
	this.exec = function exec(sql, params, callback){
		...
		process.timeBegin = 0;
		process.timeEnd = 0;
		COLOR = Math.round((Math.random() * 1) + 35);
		...
		if (object_cached){
			...
			console.info('\033[1;%smCACHE (%sms) \033[1;37m%s\33[0m', COLOR, process.timeBegin, object_cached.query);
		...
		if (this.connection.get('driver') == 'postgres'){
			...
			if (sql.match(/UPDATE/)){
				...
				params = params[0];
			}	else { for (p in params){ parameters[p] = "%L" }; parameters = ActiveSupport.values(parameters) };
			sql = Query.queryFormat(sql, parameters).replace(/`/g, '').replace(/'/g, '');
			sql = pgFormat.apply(pgFormat, ActiveSupport.flatten([sql, ActiveSupport.values(params)]));
			params = [];
		}
		...
		var tx = this.connection.conn.begin(function (){
			...
			tx.query(sql, params, function query_callback(error, data){
				...
				console.info('\033[1;%sm%s Load (%sms) \033[1;37m%s\33[0m',
					COLOR, self.name, process.timeBegin, Query.queryFormat(sql, params))
