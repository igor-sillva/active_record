				...
				if (error){
					return tx.rollback(function rollback_callback(){
						...
						if ( error.code == 'ER_PARSE_ERROR' ){
							error = new Exception.StatementInvalid(driver +' : '+error.message);
						} else if ( error.code == 'ER_BAD_FIELD_ERROR' ){
							error = new Exception.SQLFormat(driver +' : '+error.message);
						} else {
							error = new Exception.ActiveRecordError(driver +' : '+error.message);
						}
