	...
	this.once('response_callback', function _response_callback (data){
		...
		if (data.is_valid()){
			...
			var exec = data.constructor.exec((query.sql || query), params, function _exec_query_callback(error, response){
				...
				if (error || data.errors.any){
					data.constructor.emit('exception', error);
					exec.rollback();
					console.error('\033[1;%sm(%sms)\033[1;37m rollback transaction\33[0m', process.color, process.timeEnd);
					...
					console.info('\033[1;%sm(%sms) \033[1;37m commit transaction\33[0m', process.color, process.timeEnd);
				}
