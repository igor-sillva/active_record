...
function Action (){
	...
	*/
	this.create = function create(params, callback){
		...
		*/
		Action._create_callback_for.call(instance, 'create', query, params, callback);
	...
	this.update = function update(id, params, callback){
		...
		*/
		if (this instanceof this.constructor){
			...
			if (!this.changed()) return callback ? callback(null, null) : null;
		...
		var transaction = this.find(id, function _update_transaction(error, data){
			...
			if(data[0]){
				...
				*/
				...
				/* Check if have changes */
				if (!data[0].changed()) return callback ? callback(null, null) : null;
				/**
	...
	*/
	this.update_all = function update_all(conditions, params, callback){
		...
		return this.all(conditions, params, function(error, data){
			if (error){
	...
	*/
	this['delete'] = function _delete(id, callback){
		var self = this;
		if (this instanceof this.constructor) self = this.constructor;
		var query = new Query().make_delete({
			from: self.to_s(),
			conditions: [self.primary_key +' = ?', id]
		});
		/**
		*	Callbacks (BeforeDestroy & AfterDestroy)
		...
		*/
		var instance = this instanceof this.constructor ? this : new this({id: id});
		self.emit('before_destroy', instance);
	...
	*/
	this.destroy_all = function destroy_all(conditions, callback){
		...
		return this.all(conditions, function(error, data){
			...
			if(data.length > 0){
				for(var i=0; i<data.length; i++){
					self.destroy(data[i].get(data[i].constructor.primary_key), callback);
				}
			}
		})
	}
...
Action._create_callback_for = function _create_callback_for (action_name, query, params, callback){
	...
	this.once('response_callback', function _response_callback (data){
		...
		if (data.is_valid()===true){
			...
			switch(action_name){
				case "update":
					if (ActiveSupport.keys(params).length==0) return callback ? callback.call(data, null, null, null) : null;
				...
				case "destroy":
					if (data.constructor.cache)
						data.constructor.cache.remove_record_by(data.constructor.primary_key, data.get(data.constructor.primary_key));
					data.destroyed = true;
					params = query.values
					break;
			}
