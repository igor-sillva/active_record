...
function Action (){
	...
	*/
	this.destroy = function destroy(id, callback){
		...
		var primary_key = this.primary_key ? this.primary_key : this.constructor.primary_key;
		var conditions = {}; conditions[primary_key] = id;
		var options = {
			from: this.to_s ? this.to_s() : this.constructor.to_s(),
	...
	*/
	this.delete_all = function delete_all (conditions, callback){
		...
		var fakeInstance = new this();
		if (Array.isArray(conditions)){
			fakeInstance.set(conditions[0].split(' ')[0], conditions[1]);
		} else {
			for (c in conditions){
				c = ActiveSupport.Inflector.underscore(c.split('_')[0]);
				fakeInstance.set(c, conditions[c]);
			}
		}
		this.cache.clear();
		this.emit('before_destroy', fakeInstance);
		Action._create_callback_for.call(fakeInstance, 'destroy', query, fakeInstance.get(this.primary_key), callback);
		fakeInstance.emit('before_destroy', fakeInstance);
		return fakeInstance;
	}
}
...
Action._create_callback_for = function _create_callback_for (action_name, query, params, callback){
	...
	var data = this;
	/* Emit response to the instance listen and receive a response */
	...
	this.once('response_callback', function _response_callback (data){
		...
		if (data.is_valid()){
			...
			if (action_name == 'update'){
				...
				if (data.constructor.cache){
					data.constructor.cache.remove_record_by(
						...
						data.get(data.constructor.primary_key || data.constructor.foreign_key)
					);
				}
			...
			var exec = data.constructor.exec((query.sql || query), params, function _exec_query_callback(error, response){

				return callback ? callback.call(data, error, response, data) : response;
				timeEnd = Math.round(((new Date - process.timestamp)/1000)*100) / 100;
				if (error || data.errors.any || response.warningCount > 0){
					data.constructor.emit('exception', error);
					exec.rollback();
					console.error('\033[1;%sm(%sms)\033[1;37m rollback transaction\33[0m', process.color, timeEnd);
					return callback ? callback.call(data, error, null, data) : response;
				} else {
					exec.commit();
					if (response.insertId) data.set(data.constructor.primary_key, response.insertId);
					console.info('\033[1;%sm(%sms) \033[1;37m commit transaction\33[0m', process.color, timeEnd);
					/* Fire events */
			...
			callback ? callback.call(data, null, null, data) : undefined;
		}
	})
}
