...
function Action (){
	...
	*/
	this.create = function create(params, callback){
		...
		*/
		var instance = params instanceof this ? params : new self(params);
		_create_callback_for('create', query, params_, callback, instance);
	...
	this.update = function update(id, params, callback){
		...
		return this.find(id, function(data){
			if( data[0] ){
				...
				*/
				_create_callback_for('update', query, data[0][self.primary_key], callback);
				self.emit('before_update', data[0]);
				return data[0];
			}
		})
	}
		...
		*/
		return this.find(id, function(data){
			if(data[0]){
				...
				*/
				_create_callback_for('destroy', query, data[0]['id'], callback);
	...
	*	})
	*/
	this['delete'] = function _delete_(id, callback){
		...
		*/
		...
		_create_callback_for('destroy', query, id, callback);
		self.emit('before_destroy', this instanceof self ? this : instance);
		return instance;
	}
	...
	*/
	var _create_callback_for = function _create_callback_for (callback_name, query, params, callback, instance){
		var after = 'after_'+ callback_name;
		...
		instance.on('response_callback', function _response_callback (data){
			...
			if (data.errors.any){
				...
				setTimeout(function() {
					...
					if (!data.errors.any){
						return self.exec_query(query, params, function _callback (data){
