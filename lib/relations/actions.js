	...
	this.update = function update(id, params, callback){
		...
		return this.find(id, function(data){
			if(data[0]){
				...
				*/
				_create_callback_for('update', query, params, callback, data[0]);
		...
		*/
		return this.find(id, function(data){
			if(data[0]){
				...
				*/
				_create_callback_for.bind(self)('destroy', query, data[0]['id'], callback, data[0]);
	...
	*/
	this['delete'] = function _delete(id, callback){
		...
		*/
		...
		// return instance;
		return this.exec_query(query, id, function _delete(data){
	...
	var _create_callback_for = function _create_callback_for (callback_name, query, params, callback, instance){
		...
		instance.on('response_callback', function _response_callback (data){
			if (callback_name=="update"){
				...
				// params = data && data.values ? data.values() : params;
				params = data.to_object()
			}

			if (!data.errors.any && data.valid){

				return instance.constructor.exec_query(query, params, function _callback_create_callback_for(data){
					if (data.affectedRows > 0 || data.changedRows > 0){
						instance.emit(callback_after_name, callback_name == 'create' ? data : instance);
					}
					return callback ? callback(data) : data;
				})
