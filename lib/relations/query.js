	...
	*/
	this.make_select = function ( options ){
		...
		if( options.conditions ){
			...
			*	Operator: OR|AND
			*/
			opt.operator = opt.operator || "AND";
			if(Array.isArray(where)){
				where = where.join(" "+ opt.operator +" ");
			}
		...
		*/
		if( options.include || options.join ){
			var inc = options.include || options.join ;
				...
				join = inc.join || inc;
				on = inc.on || inc +'.'+ from.singularize() +'_id = '+ from +'.id';
	...
	this.make_update = function ( options ){
		...
		if( !options.params && typeof(options.params)!="object" )
			throw new Error.SQLFormat('Can\'t SET '+ options.params).toString();
