...
function Finder (){
	...
	*/
	this.find = function find(id, options, values, callback){
		...
		return self.exec_query( query, values, function (data){
			if (data.length > 0){
				for (d in data){
					data[d] = new self(data[d]);
				}
			}
	...
	*/
	this.find_by_sql = function find_by_sql(sql, params, callback){
		return self.exec_query(sql, params, function (data){
			if (data.length > 0){
				for (d in data){
					data[d] = new self(data[d]);
				}
	...
	*/
	this.join = function join(table_name, options, callback){
		...
		query = new Query().make_select(options);
		return self.exec_query(query, function (data){
			if (data.length > 0){
				for (d in data){
					data[d] = new self(data[d]);
				}
