...
function Finder (){
	...
	*/
	this.find = function find(id, conditions, callback){
		var options = options || {}, conditions = conditions || {}, force = false;
		/* Reload the cache */
		if (conditions.force == true)	{
			this.cache.clear();
			delete conditions.force;
		}
		/* If find({select: '*', ...}); */
		...
		if (typeof(id) == "string"){
			...
			switch (id){
				...
				case 'last': /* Finder.last */
					...
					break;
				case 'all': /* Finder.all */
					options.select = options.select || this.to_s() + ".*";
					break;
				default:
		...
		if (typeof(id) == 'number') return this.find_by_id(id, options, callback);
		...
		this.find_by_sql(query.sql, query.values, callback);
		return this;
	}
	...
	*/
	this.find_by_id = function find_by_id (id, options, callback){
		var conditions = {}; conditions[this.primary_key] = id
		options = options || {}; options.conditions = conditions;
	...
	*/
	this.exists = function exists(id_or_object, callback){
		...
		this.find_by_sql(query.sql, query.values, function _exists_transaction(error, data){
			var resp = data[0] && data[0]['1']==1 ? true : false;
			self.emit('after_find', data);
			return callback ? callback.call(this, error, resp) : resp;
		});
		return this;
	}
	...
	*/
	this.join = function join(table_name, options, callback){
		options = options || {};
		var conditions = ActiveSupport.clone(options.conditions || options);
		if(typeof(options)=="function"){
			callback = options;
			options = {};
		}

		if (!options.conditions) { options.conditions = conditions };

		options.from = this.to_s();
		options.select = table_name + ".*";
		options.join = table_name;
		options.primary_key = this.primary_key;
		options.foreign_key = this.foreign_key;
	...
	*/
	this.find_by_sql = function find_by_sql(sql, values, callback){
		...
		var transaction = this.exec(sql, values, function _find_by_sql_transaction(error, data){
			...
			return callback ? callback.call(transaction, error, data) : data;
		}.bind(this));
