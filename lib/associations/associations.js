...
function Association (){
			...
			self.exec_query(query, params, function (data){
				if (data.length > 0){
					for (d in data){
						...
						if (dependent){
							data[d].on('before_destroy', function _destroy_dependents(data){
								Klass[dependent+"_all"](Klass.primary_key.concat(" = ?"), data[self.primary_key]);
							})
						}
					}
				}
	...
	*/
	this.has_many = function has_many (association, options) {
		...
		this.prototype[association] = function(callback){
			Klass.prototype.set(foreign_key, this[self.primary_key]); // TODO
			...
			self.exec_query(query, params, function (data){
				if (Klass && data.length > 0){
					...
					if (dependent){
						data[d].on('before_destroy', function _dependent_destroy(data){
							Klass[dependent+"_all"](Klass.primary_key.concat(" = ?"), data[self.primary_key]);
						})
					}
				}
