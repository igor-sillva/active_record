...
function Association (){
	...
	*/
	this.belongs_to = function belongs_to (association, options) {
		...
		this.before_destroy(function _destroy_all_dependents(record){
			if (dependent){
				record.before_destroy(function _destroy_all_dependents(record){
					if (!record.errors.any){
						if (has_one && dependent == 'nullify'){
							...
							Klass[dependent+"_all"]([Klass.primary_key.concat(" = ?"), record.get(record.constructor.primary_key)]);
						}
					}
				})
			}
...
Association.check_if_model_exists = function check_if_model_exists (association){
	...
	try {
		...
		/* Try aagain require the module in the parent path */
		Klass = require(path.join(path_to_models, '..', association));
	}
