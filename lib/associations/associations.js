...
function Association (){
	...
	*/
	this.belongs_to = function belongs_to (association, options) {
		...
		this.once('before_destroy', function _destroy_dependents(record){
			if (dependent){
				record.once('before_destroy', function _destroy_dependents(record){
					if (!record.errors.any){
						if (has_one && dependent == 'nullify'){
							...
							Klass[dependent+"_all"]([Klass.primary_key.concat(" = ?"), record.get(self.foreign_key)]);
						}
					}
				})
			}
		...
		this.prototype[association] = function _belongs_to_association(callback){
			...
			Klass._extra_parameters = [Klass.foreign_key +" = "+ record.get(record.constructor.primary_key)]
			return Klass;
		}
	}
				...
				record.once('before_destroy', function _destroy_dependents(record){
					if (!record.errors.any){
						if (dependent=='nullify'){
							...
							Klass[dependent+"_all"]([Klass.foreign_key.concat(" = ?"), record.get(self.primary_key)]);
						}
					}
				})
			}
...
Association.check_if_model_exists = function check_if_model_exists (association){
	moduleExports = process.mainModule.exports[ActiveSupport.Inflector.humanize(association)];
