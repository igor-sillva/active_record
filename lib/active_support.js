...
var ActiveSupport = {
	...
	*
	*	Example:
	*	ActiveSupport.extend({}, {a: 2}, [...])
	*/
	extend: function extend(destination){
		for (var i=1; i<arguments.length, source=arguments[i]; i++){
	...
	*/
	...
	/* Thanks! to: https://github.com/jashkenas/underscore/blob/master/underscore.js#L501 */
	flatten: function flatten(input, shallow, strict, startIndex){
		...
		for (var i = startIndex || 0, length = input && input.length; i < length; i++) {
			value = input[i];
			if (value && value.length >= 0 && (Array.isArray(value) || Object.prototype.hasOwnProperty.call(value, 'callee'))) {
	...
	/*
	* Thanks to https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/freeze?redirectlocale=en-US&redirectslug=JavaScript%2FReference%2FGlobal_Objects%2FObject%2Ffreeze
	* Make object fully immutable
	* @param {Object} object
	*/
	freeze: function freeze (object){
		...
		for (key in object) {
			prop = object[key];
			if (!object.hasOwnProperty(key) || !(typeof prop === 'object') || Object.isFrozen(prop)) {
			  continue;
			}
			ActiveSupport.freeze(prop);
		}
	},

	Inflector : {
		/*
		* Configure using a callback
		* @param {String} locale
		* @param {Function} callback
		*/
		inflections: function inflections (locale, callback){
			if (callback){
				ActiveSupport.Inflector.Inflections.clear('all'); /* Clear all inflections */
				ActiveSupport.Inflector.locale = locale || ActiveSupport.Inflector.locale; /* Set Locale */

				return callback({
					plural: function plural(regex, replace){
						ActiveSupport.Inflector.Inflections.plural_rules.push([regex, replace]);
					},

					singular: function singular(regex, replace){
						ActiveSupport.Inflector.Inflections.singular_rules.push([regex, replace]);
					},
					...
					*/
					uncountable: function uncountable(words){
						ActiveSupport.Inflector.Inflections.uncountable_words = words;
					}
				});
			}
		},
		...
		* @property {String} */
		...
		camelize: function camelize(string, lowFirstLetter) {
