...
AttributesMethods.prototype = {
	...
	changed: function changed(key){
		...
		for (key in params){
			if (this.get(key) != params[key]){
				if (!changes.hasOwnProperty(key)) changes[key] = [];
	...
	*/
	save: function save( callback ) {
		if (this.new_record == false && !this.destroyed) return this.update_attributes(this.to_object(), callback);
		if (this.errors.any) {
		...
		var saved_record = this.constructor.create(this.to_object(), function (error, response, record) {
			...
			return callback ? callback.call(this, error, response, record) : response;
		}.bind(this));

		if (!saved_record.errors.any){
			if (this.password_digest) this.password_digest = saved_record.password_digest;
	...
	*/
	update_attributes: function update_attributes(attributes, callback) {
		...
		var updated_record = this.constructor.update.call(this, this.get(this.constructor.primary_key), attributes,
	...
	*/
	destroy: function destroy( callback ) {
		...
		var destroyed_record = this.constructor.destroy.call(this, this.get(this.constructor.primary_key),
		function(error, response, record){
			if (error) return callback ? callback.call(this, error, null) : error;
			if (response){
		...
		if (destroyed_record) this.destroyed = destroyed_record.destroyed;
		Object.freeze(this);
